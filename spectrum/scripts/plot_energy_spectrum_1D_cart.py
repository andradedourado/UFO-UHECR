...
def plot_energy_spectrum_over_time(sim):
    ...
    for i in range(118, len(t_i) - 1, 10):
        mask = (T > t_i[i]) & (T < t_i[i+1]) & (X > 0) & (X < get_position_mask(sim))
...
def plot_energy_spectrum_stationary(sim):
    ...
    for i in range(118, len(t_i) - 1, 10):
        mask = (T <= t_i[i+1]) & (X > 0) & (X < get_position_mask(sim))
        candidates = data[mask,:]
...
if __name__ == '__main__':
    ...
    for sim in ['1D_cart_Dconst', '1D_cart_Denergy']:
        plot_energy_spectrum_over_time(sim)
        plot_energy_spectrum_stationary(sim)
