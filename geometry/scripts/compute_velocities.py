...
# ----------------------------------------------------------------------------------------------------
def compute_v_ts(t_age): # Termination shock, [t_age] = yr 
...
def write_velocities():
    ...
    t_age = np.logspace(np.log10(10 * free_expansion_time()), 4, num = 100)
    np.savetxt(f"{RESULTS_DIR}/velocity_forward_shock.dat", np.column_stack((t_age, compute_v_fs(t_age))), fmt = "%.15e")
    np.savetxt(f"{RESULTS_DIR}/velocity_termination_shock.dat", np.column_stack((t_age, compute_v_ts(t_age))), fmt = "%.15e")
